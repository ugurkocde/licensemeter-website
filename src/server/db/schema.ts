...
export const mspAccounts = pgTable(
  "msp_accounts",
  {
    ...
    name: text("name"),
    /** Entra object id of the creator who owns and manages this MSP account. */
  ...
  (t) => [
    ...
    // accounts past the read-then-insert race. Unassigned owners cannot authenticate.
...
export const tenants = pgTable(
  "tenants",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    /**
     * Entra tenant id (tid claim). A workspace can exist before a Microsoft
     * tenant is connected. Set when an msConnections row is added
    ...
    /** Cached subscription status, mirrored from the webhook. */
    subscriptionStatus: text("subscription_status").$type<SubscriptionStatus>(),
    /** Cached current_period_end; the paid-access horizon. Null on trial. */
    paidUntil: timestamp("paid_until", { withTimezone: true }),
    /** Comp / grandfather flag, NEVER written by the webhook. Set => always entitled. */
...
export const joinRequests = pgTable(
  "join_requests",
  {
    ...
    email: text("email").notNull(),
    /** Entra object id and home tenant of the requester; null on historical requests. */
    oid: text("oid"),
    tid: text("tid"),
    name: text("name"),
    status: text("status")
...
export const dpaAcceptances = pgTable(
  "dpa_acceptances",
  {
    ...
    /** Identity recorded for the accepting owner or admin. */
...
export const dpaAgreements = pgTable(
  "dpa_agreements",
  {
    ...
    /** Identity recorded for the owner who signed in the portal. */
