...
import { setTrialReminders } from "~/server/actions";
import type { ActionResult } from "~/server/actions";

/**
 * Trial-reminder email toggle for the Workspace card. Same optimistic
 * save-on-change pattern as MonthlyReportToggle, plus a live region so the
 * result is announced instead of discarded.
 */
export const TrialRemindersToggle = ({ initial }: { initial: boolean }) => {
        ...
        <input
          ...
          onChange={(e) => {
            ...
            startTransition(async () => {
              try {
                const res = await setTrialReminders(next);
                setResult(res);
                if (!res.ok) {
                  setCurrent(initial);
                  return;
