...
import { setLeakAlerts } from "~/server/actions";
import type { ActionResult } from "~/server/actions";

/**
 * Leak-alert email toggle for the Workspace card. Same optimistic
 * save-on-change pattern as CurrencySelect, plus a live region so the
 * result is announced instead of discarded.
 */
export const LeakAlertsToggle = ({ initial }: { initial: boolean }) => {
        ...
        <input
          ...
          onChange={(e) => {
            ...
            startTransition(async () => {
              try {
                const res = await setLeakAlerts(next);
                setResult(res);
                if (!res.ok) {
                  setCurrent(initial);
                  return;
