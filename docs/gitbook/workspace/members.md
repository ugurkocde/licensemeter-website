...
Where email delivery is configured, the colleague receives a sign-in link. Otherwise, direct them to the installation's sign-in page. They must use the invited identity and complete any requested email verification. On self-hosted Entra installations this is Microsoft sign-in; the hosted service uses its available account sign-in methods.
...
On hosted WorkOS sign-in, eligible colleagues with a verified company email can be matched to the workspace associated with that domain. This is governed by **Who can join**; it is not unrestricted access for everyone using Microsoft sign-in. Public email providers are not treated as a shared company domain.
...
Only an Owner can change this setting. It appears for the workspace that holds an eligible company domain, not for every workspace. If absent, use invitations. This domain-join feature is separate from the Entra-only self-hosted sign-in path.
